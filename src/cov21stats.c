/* COV21STATS - DeepMetaPSICOV Alignment Covariance Matrix Program - by David T. Jones June 2017 */
...
int             main(int argc, char **argv)
{
    ...
    double (**pab)[21][21];
    double aacomp[21], sumj[21], sum, score, pa[MAXSEQLEN][21], pav[MAXSEQLEN][21], di, pdir, hahb, hx, hy, hxy, mi, mip, z, oldvec[21], change, wtsum, idthresh=0.38, potsum;
    float *weight, val;
    char            buf[4096], name[512], seq[MAXSEQLEN], qseq[160], *cp;
    FILE *ifp, *singofp, *pairofp;

    if (argc != 3)
	fail("Usage: cov21stats alnfile covaroutfile");
    ...
    pab = allocmat(seqlen, seqlen, sizeof(**pab));
